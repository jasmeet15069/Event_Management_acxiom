...
Class Action {
	...
	function login(){
		extract($_POST);
		$qry = $this->db->query("SELECT * FROM event_users where username = '".$username."' and password = '".md5($password)."' ");
		if($qry->num_rows > 0){
	...
	function save_user(){
		...
		$chk = $this->db->query("Select * from event_users where username = '$username' and id !='$id' ")->num_rows;
		if($chk > 0){
			return 2;
			exit;
		}
		if(empty($id)){
			$save = $this->db->query("INSERT INTO event_users set ".$data);
		}else{
			$save = $this->db->query("UPDATE event_users set ".$data." where id = ".$id);
		}
		if($save){
			return 1;
		}
	...
	function delete_user(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM event_users where id = ".$id);
		if($delete)
			return 1;
	}
	function signup(){
		...
		$data .= ", password = '".md5($password)."' ";
		$data .= ", type = 3";
		$chk = $this->db->query("SELECT * FROM event_users where username = '$email' ")->num_rows;
		if($chk > 0){
			return 2;
			exit;
		}
			$save = $this->db->query("INSERT INTO event_users set ".$data);
		if($save){
			$qry = $this->db->query("SELECT * FROM event_users where username = '".$email."' and password = '".md5($password)."' ");
			if($qry->num_rows > 0){
				foreach ($qry->fetch_array() as $key => $value) {
					if($key != 'passwors' && !is_numeric($key))
						$_SESSION['login_'.$key] = $value;
